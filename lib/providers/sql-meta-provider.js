...
class SQLMetaProvider {
  ...
  getTableNames(editor, tablePrefix, schema) {
    ...
    let valid = tables.filter((table) => {
      if (schema) {
        return (table.schemaName || table.schemaName === schema) && table.name.startsWith(tablePrefix);
      } else if (tablePrefix) {
        return table.name.startsWith(tablePrefix) || (table.schemaName && table.schemaName.startsWith(tablePrefix));
      }
    });
    return valid.map((table) => {
      return {
        text: (schema || table.schemaName === 'public' || !table.schemaName) ? table.name : table.schemaName + '.' + table.name,
