...
class DbFactory {
  getSupportedDatabases() {
    return [
      ...
      {name: 'Firebird', prefix: 'firebird', port: 3050}
    ];
  }
  ...
  createDataManagerForUrl(url) {
    ...
    switch (dbConfig.protocol) {
      ...
      case 'firebird': {
        var FirebirdManager = require('./firebird-manager');
        dbConfig.database = dbConfig.database.replace(/\+/g,'\\').replace('!',':');
        return new FirebirdManager(dbConfig);
      }
      default:
        return null;
    }
